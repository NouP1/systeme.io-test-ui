...
import { pricePlansData } from "@/lib/data/mockData";
import type { RowData, SortType } from "@/lib/types/data";
import { useEffect, useState } from "react";

export function PricePlans() {
  const savedData = JSON.parse(
    localStorage.getItem("pricePlans_tableData") || "null"
  );
  const { headers } = useTableDataUi(pricePlansData);
  const initialData = savedData ?? pricePlansData;
  ...
  useEffect(() => {
    if (!isEditMode) {
      const saved = localStorage.getItem("pricePlans_tableData");
      if (saved) {
        setEditableData(JSON.parse(saved));
      } else {
        setEditableData(pricePlansData);
      }
    }
  }, [isEditMode]);
  ...
  const handleDataChange = (rowIndex: number, key: string, value: any) => {
    setEditableData((prev: any) => {
      ...
      localStorage.setItem("pricePlans_tableData", JSON.stringify(newData));
      return newData;
    });
  };
